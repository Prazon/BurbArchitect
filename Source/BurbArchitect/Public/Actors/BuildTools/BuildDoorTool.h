...
#include "BuildDoorTool.generated.h"

/**
 * Build Door Tool - Places door portals on walls with preview
 */
UCLASS()
class ABuildDoorTool : public ABuildPortalTool
{
	GENERATED_BODY()
...
protected:
	// Preview door actor shown during placement - Replicated so all clients see it
	UPROPERTY(BlueprintReadOnly, VisibleAnywhere, Replicated)
	APortalBase* PreviewDoor;

	// Cached loaded class from ClassToSpawn
	UPROPERTY()
	TSubclassOf<APortalBase> ClassToPlace;

	// Helper to spawn or update preview door
	void UpdatePreviewDoor();

	// Helper to destroy preview door
	void DestroyPreviewDoor();

...
// Sets default values
ABuildWallTool::ABuildWallTool(): CurrentWallComponent(nullptr)
{
	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = false;
}

void ABuildWallTool::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
}

// Called when the game starts or when spawned
void ABuildWallTool::BeginPlay()
{
	Super::BeginPlay();

	// Initialize wall height from LotManager if available
	if (CurrentLot)
	{
		WallHeight = CurrentLot->DefaultWallHeight;
	}
}

void ABuildWallTool::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
int64 ABuildWallTool::GenerateWallCacheKey(int32 StartRow, int32 StartColumn, int32 EndRow, int32 EndColumn)
{
...
void ABuildWallTool::ClearPreviewCache()
{
...
// Called every frame
void ABuildWallTool::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

void ABuildWallTool::CreateWallPreview(int32 Level, int32 Index, FVector StartLocation, FVector Direction)
{
...
void ABuildWallTool::Move_Implementation(FVector MoveLocation, bool SelectPressed, FHitResult CursorWorldHitResult, int32 TracedLevel)
{
		if (!CurrentLot)
		{
			return;
...
void ABuildWallTool::Click_Implementation()
{
	PreviousLocation = GetActorLocation();
...
void ABuildWallTool::Drag_Implementation()
{
...
void ABuildWallTool::BroadcastRelease_Implementation()
{
	if (DragCreateWallArray.IsValidIndex(0))
	{
		...
		if (!BuildServer)
		{
			UE_LOG(LogTemp, Error, TEXT("BuildWallTool: BuildServer subsystem not found"));
			return;
		}
		...
		for(UWallComponent* Wall : DragCreateWallArray)
		{
			...
			else
			{
				...
				{
					// No existing wall found - store for batch creation
					WallsToCreate.Add(Wall);
				}
			}
		}

		// Start batch operation if we have multiple operations
		bool bNeedsBatch = (WallsToRemove.Num() + WallsToCreate.Num()) > 1;
		if (bNeedsBatch)
		{
			FString BatchDesc = bDeletionMode ?
				FString::Printf(TEXT("Delete %d Walls"), WallsToRemove.Num()) :
				FString::Printf(TEXT("Build %d Walls"), WallsToCreate.Num());
			BuildServer->BeginBatch(BatchDesc);
		}

		// Batch process removals using BuildServer
		for (FWallSegmentData* WallToRemove : WallsToRemove)
		{
			BuildServer->DeleteWall(*WallToRemove);
		}
...
FVector ABuildWallTool::SnapToAllowedDirection(const FVector& Direction) const
{
...
FVector ABuildWallTool::ConstrainToLockedDirection(const FVector& StartPos, const FVector& CurrentPos) const
{
